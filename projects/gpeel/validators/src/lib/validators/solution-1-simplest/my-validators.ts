/**
 * GAUTHIER PEEL : copied form version 16.1.0 of @angular/forms
 * MyValidators now compute an error messages in the same time they are executed
 *
 * https://github.com/angular/angular/blob/main/packages/forms/src/validators.ts
 *
 * @license
...
import {ɵisPromise as isPromise, ɵisSubscribable as isSubscribable, ɵRuntimeError as RuntimeError} from '@angular/core';
import {
  AbstractControl,
  AsyncValidator,
  AsyncValidatorFn,
...
// export const NG_ASYNC_VALIDATORS = new InjectionToken<Array<Validator | Function>>('NgAsyncValidators');
...
export class MyValidators {
   ...
   *
   */
  static pattern(pattern: string | RegExp, errorKey = 'pattern'): ValidatorFn {
    return patternValidator(pattern, errorKey);
  }
...
export function requiredValidator(control: AbstractControl): ValidationErrors | null {
  return isEmptyInputValue(control.value) ? {'required': {msg: 'the field could not be empty'}} : null;
}
    ...
    return hasValidLength(control.value) && control.value.length > maxLength ?
      {
        'maxlength': {
          ...
          'actualLength': control.value.length,
          msg: `the length should be less than ${length} characters`
        }
      } :
      null;
  };
 ...
 */
export function patternValidator(pattern: string | RegExp, errorKey: string): ValidatorFn {
